...
func TestBulkIndexer_413PayloadTooLarge(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stat, err := indexer.Flush(t.Context())
			require.NoError(t, err)
...
func TestBulkIndexer_413PayloadTooLarge_MaxRecursionDepth(t *testing.T) {
	// This test verifies that we don't get infinite recursion
	var requestCount int64
	...
	client := docappendertest.NewMockElasticsearchClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		// Always return 413 to force maximum recursion
		// This will continue until we hit the max recursion depth limit
		w.WriteHeader(http.StatusRequestEntityTooLarge)
		fmt.Fprintf(w, `{"error":{"type":"request_entity_too_large","reason":"Request entity too large with %d docs"}}`, len(docs))
	})
	...
	// Add enough documents to trigger recursion but not so many we reach single docs
	// With max depth 10, we'll split 2^10 = 1024 times max, so use more than that
	for i := 0; i < 2048; i++ {
		require.NoError(t, indexer.Add(docappender.BulkIndexerItem{
			Index: "test-index",
			Body:  newJSONReader(map[string]any{"id": i}),
		}))
	}

	// Should eventually return an error about max recursion depth
	_, err = indexer.Flush(t.Context())
	require.Error(t, err)
	assert.Contains(t, err.Error(), "maximum recursion depth")

	// We should hit the recursion limit with the expected number of requests
	// With depth 10, we should get 2^10 = 1024 leaf nodes, plus internal nodes
	reqCount := atomic.LoadInt64(&requestCount)
	assert.True(t, reqCount > 1000, "Expected more than 1000 requests, got %d", reqCount)
	assert.True(t, reqCount < 3000, "Expected less than 3000 requests, got %d", reqCount)
}
